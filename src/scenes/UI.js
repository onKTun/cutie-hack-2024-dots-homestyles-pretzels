...
export class Game extends Scene {
  ...
  create() {
    ...
    let money = 0;
    let score = 0
    let junkLvl1 = Phaser.Math.Between(100, 400);
    let junkLvl2 = Phaser.Math.Between(200, 500);


    this.add
      .image(this.scale.width / 2, this.scale.height / 2, "background")
      .setDisplaySize(this.scale.width, this.scale.height);
      this.add.image(500, 300, "satellite").setScale(2);
// Header
     let rect1 = this.add.rectangle(this.scale.width/2, 80, this.scale.width, 200, '0x0B1A33', 1);
    
    //sub menu 
     let rect2 = this.add.rectangle(this.scale.width/2, 250, this.scale.width, 140, '0x3C4D6A', 1);
     ...
     let dropdownArrow = this.add.image(this.scale.width/2, 205, "dropdownIcon");
     ...
     buttonClose.strokeRect(this.scale.width/2-158/2, 349-57/2, 158, 57);
  ...
  buttonContainer.on('pointerdown', () => {
   ...
   closeArrow.visible = true;
    ...
    satelliteButton.visible = true;
    satelliteText.visible = true;
    ...
    //sub-menu
    let spacecraftText = this.add.text(this.scale.width/5, 215, "Spacecraft", {fontSize: 20}).setOrigin(0.5, 0.5);
    let junkRemovalText = this.add.text(this.scale.width/1.32, 215, "Junk Removal", {fontSize: 20}).setOrigin(0.5, 0.5);
    ...
    let spaceshipText = this.add.text(110, 275, "Spaceship", {fontSize: 16}).setOrigin(0.5, 0.5);
    ...
    let satelliteText = this.add.text(300, 275, "Satellite", {fontSize: 16}).setOrigin(0.5, 0.5);
    ...
    let netText = this.add.text(620, 275, "Net", {fontSize: 16}).setOrigin(0.5, 0.5);


    const magnetButton = this.add.graphics();
    magnetButton.fillStyle(0x0B1A33, 1); 
    magnetButton.fillRect(720, 270-37/2, 120, 50); 
    const magnetContainer = this.add.container(660, 275);
    magnetContainer.setSize(120, 50); 
    magnetContainer.setInteractive({ useHandCursor: true  });
    let magnetText = this.add.text(780, 275, "Magnet", {fontSize: 16}).setOrigin(0.5, 0.5);


    const emButton = this.add.graphics();
    emButton.fillStyle(0x0B1A33, 1); 
    emButton.fillRect(880, 270-37/2, 120, 50); 
    const emContainer = this.add.container(660, 275);
    emContainer.setSize(120, 50); 
    emContainer.setInteractive({ useHandCursor: true  });
    let emText = this.add.text(940, 275, "E.M.", {fontSize: 16}).setOrigin(0.5, 0.5);




    // Title
      let year1 = this.add.text(this.scale.width/2, 75, year, {fontSize: 64}).setOrigin(0.5, 0.5);
      let money1 = this.add.text(this.scale.width/2, 125, "$" + money, {fontSize: 36}).setOrigin(0.5, 0.5);

    // Goal Progress
      let spaceProgress = this.add.text(this.scale.width/1.25, 75, "Spaceships landed: ", {fontSize: 24}).setOrigin(0.5, 0.5);
      let spaceProgressNumber = this.add.text(this.scale.width/1.25, 110, score + "/100", {fontSize: 24}).setOrigin(0.5, 0.5);
    ...
    let spaceJunk1 = this.add.text(this.scale.width/4.1, 90, "Space Junk Lvl 1: " + junkLvl1 + "\nSpace Junk Lvl 2: " + junkLvl2, {fontSize: 20}).setOrigin(0.5, 0.5);
